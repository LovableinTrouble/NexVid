...
function validatePayload(path: string, body: any): string | null {
  ...
  if (path === 'auth/register') {
    ...
    if (
      ...
      username.trim().length < LIMITS.USERNAME_MIN ||
      username.trim().length > LIMITS.USERNAME_MAX
    ) {
  ...
  if (path === 'user/feedback' && body.subject && body.message) {
    if (
      ...
      body.subject.trim().length < LIMITS.FEEDBACK_SUBJECT_MIN ||
      body.subject.trim().length > LIMITS.FEEDBACK_SUBJECT_MAX
    ) {
      return `Subject must be between ${LIMITS.FEEDBACK_SUBJECT_MIN} and ${LIMITS.FEEDBACK_SUBJECT_MAX} characters`;
    }
    if (
      typeof body.message !== 'string' ||
      body.message.trim().length < LIMITS.FEEDBACK_MESSAGE_MIN ||
      body.message.trim().length > LIMITS.FEEDBACK_MESSAGE_MAX
    ) {
  ...
  if (path === 'user/feedback/messages' && body.message) {
    if (
      typeof body.message !== 'string' ||
      body.message.trim().length < LIMITS.FEEDBACK_REPLY_MIN ||
      body.message.trim().length > LIMITS.FEEDBACK_REPLY_MAX
    ) {
  ...
  if (path === 'user/profile' && body.username) {
    if (
      ...
      body.username.trim().length < LIMITS.USERNAME_MIN ||
      body.username.trim().length > LIMITS.USERNAME_MAX
    ) {
...
export async function ANY(req: NextRequest, { params }: { params: Promise<{ path?: string[] }> }) {
  try {
    ...
    const path = pathSegments && pathSegments.length > 0 ? pathSegments.join('/') : 'proxy';
    ...
    if (
      (path === 'auth/login' || path === 'auth/register' || path === 'auth/change-password') &&
      workerResponse.ok
    ) {
