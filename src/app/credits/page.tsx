      ...
      <div className="space-y-12">
        {SECTIONS.map((section) => (
          <section key={section.title}>
            <h2 className="text-text-muted/60 mb-4 px-1 text-[12px] font-bold uppercase tracking-widest">
              {section.title}
            </h2>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-2">
              {section.items.map((item) => (
                <div
                  key={item.name}
                  className="glass-card glass-liquid group p-4 transition-all hover:bg-white/[0.03]"
                >
                  <a
                    href={item.url}
                    target="_blank"
                  ...
                  <p className="mt-1 text-[12px] leading-snug text-text-secondary">{item.desc}</p>
                </div>
              ))}
            </div>
          </section>
